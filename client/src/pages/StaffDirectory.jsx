...
import { seedDoctors } from '../data/mohSeedData.js';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {seedDoctors.map(doc => (
          <div key={doc.id} className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-moh-600 font-bold">{doc.specialty}</p>
              </div>
            </div>
            ...
            <div className="space-y-1.5 text-xs text-slate-600 dark:text-slate-300 pt-2 border-t border-slate-100 dark:border-slate-700">
              <div><b>Qualifications:</b> {doc.qualifications}</div>
              <div><b>Experience:</b> {doc.experience}</div>
              <div><b>MOH Division:</b> {doc.division}</div>
            </div>
          </div>
        ))}
      </div>
